...
public class PartyClientManager : MonoBehaviour 
{

    private PartyClient _client;
    private PartyClientResponseHandler _clientResponseHandler;

    private static PartyClientManager _instance;
    public static PartyClientManager instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                _instance = newObject.AddComponent<PartyClientManager>();
	...
	void Awake()
	{
		...
		_client = gameObject.AddComponent<PartyClient>();
        _client.clientSettings = Resources.Load<PartyClientSettingsScriptableObject>(PartyClientSettingsScriptableObject.location);
        ...
        _clientResponseHandler = new PartyClientResponseHandler(_client);
    ...
    public PartyClientSettingsScriptableObject GetClientSettings()
    {
        return _client.clientSettings;
    }

    public SNetwork.PartyNetworkPlayer GetNetworkPlayer()
    {
    ...
    {
        PartyMessaging.instance.SendServerDataSetting(ByteParser.ConvertKeyValuePairToData(data), _client.clientSocket);
    }

    public void SetUserData(KeyValuePairs data)
    {
        PartyMessaging.instance.SendUserDataSetting(ByteParser.ConvertKeyValuePairToData(data), _client.ourId, _client.clientSocket);
    }
